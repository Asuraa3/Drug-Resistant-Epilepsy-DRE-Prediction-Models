...
class EEGConnectivityFeatures:
    ...
    def __init__(self, sfreq: int = 256, channel_names: List[str] = None):
        self.sfreq = sfreq

        if channel_names is not None:
            self.all_channels = channel_names
        else:
            # Default ai 19 canali standard
            self.all_channels = []

        # Definizione dei canali frontotemporali solo se presenti
        self.frontotemporal_channels = [ch for ch in self.all_channels if any(kw in ch for kw in ['F7', 'F8', 'T3', 'T4', 'F3', 'F4'])
]
        print("canali fronto temporali:",self.frontotemporal_channels)
    ...
    def extract_frontotemporal_features(self, eeg_data: np.ndarray, channel_names: List[str]) -> Dict[str, float]:
        ...
        ft_indices = [
            ...
            if any(ft in ch for ft in self.frontotemporal_channels)
        ]

        ft_data = eeg_data[ft_indices, :]

        # Filtro nella banda theta (4-8 Hz)
        theta_data = self.bandpass_filter(ft_data, 4, 8)

        # Calcolo delle metriche di connettività
        ...
        features = {
            ...
            'ft_connectivity_density': np.mean(pli_upper_triangle > 0.1),
        }
...
class DREPredictor:
    ...
    def __init__(self, channel_names: List[str] = None):
        self.feature_extractor = EEGConnectivityFeatures(channel_names=channel_names)
...
def load_data(edf_folder: str, excel_path: str, resample: bool = False,
                       target_freq: int = 256) -> Tuple[List[np.ndarray], List[List[str]], np.ndarray]:
    """
    Carica dati EEG reali da file .edf e li prepara nel formato richiesto.

    Ritorna:
        - eeg_data_list: lista di ndarray EEG (n_channels, n_samples)
        - channel_names_list: lista dei nomi dei canali per ogni paziente
        - labels: array NumPy con etichette associate
    """
    x = []
    channel_names_list = []
    y = []
    ...
    for filename in os.listdir(edf_folder):
        if filename.endswith('.edf') and filename in label_dict:
            ...
            data = raw.get_data()
            temp_data.append((data, raw.ch_names, label_dict[filename]))
            min_len = min(min_len, data.shape[1])

    for data, ch_names, label in temp_data:
        x.append(data[:, :min_len])  # Taglia tutte le registrazioni alla stessa lunghezza
        channel_names_list.append(ch_names)
        y.append(label)

    return x, channel_names_list, np.array(y)
